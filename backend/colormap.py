#colormap for density of the data points
import pandas as pd
import numpy as np
...
x_grid = np.linspace(2.0, 8.0, 100)
y_grid = np.linspace(8.0, 28.0, 100)
X, Y = np.meshgrid(x_grid, y_grid)
fig, ax = plt.subplots(num="Anomaly Detection")
# 2. Evaluate the KDE density on this structured grid
grid_coords = np.vstack([X.ravel(), Y.ravel()])
Z = kde(grid_coords).reshape(X.shape)

# 3. Plot the background density correctly
c = ax.pcolormesh(X, Y, Z, cmap="RdBu", shading='auto', alpha=0.3)


t = 1

fig.colorbar(c, label='Density',ax=ax)
