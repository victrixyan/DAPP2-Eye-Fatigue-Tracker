...
color_gradient=((anomaly_score*(-1)+np.ones(200))*100).astype(int)
fig1, ax1 = plt.subplots(num="Training Data")
ax1.scatter(eye_data_df[:, 0], eye_data_df[:, 1], label='Data Points',c =color_gradient,cmap="Reds",marker="o")
ax1.set_xlabel('Pupil Size (mm)')
ax1.set_ylabel('Blink Rate (bpm)')
...
ax.scatter(eye_data_df[:, 0], eye_data_df[:, 1],
           marker="o", color="b", label='Data Points')
